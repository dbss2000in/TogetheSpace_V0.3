...
# --- 1. CONFIGURATION & ROBUST CREDENTIAL LOADER (BASE64 SECRETS METHOD) ---
SCOPES = [
    ...
    try:
        ...
        try:
            ...
            if not df_users.empty:
                df_users.columns = df_users.columns.str.strip().str.lstrip("\ufeff")
            ...
            if submit_pass:
                if not old_pass or not new_pass:
                    ...
                    try:
                        ...
                        cell = us.find(current_user)
                        if cell:
                            us.update_cell(cell.row, 2, hash_password(new_pass))
                            st.cache_data.clear()
                ...
                if st.form_submit_button("Create Account") and u_name.strip():
                    try:
                        ...
                        try:
                            ...
                            us = client.open_by_key(MASTER_SHEET_ID).add_worksheet(title="Users", rows="100", cols="4")
                            us.append_row(["Username", "Password Hash", "Organization", "Role"])
